...
function UpdateHeadman() {
  ...
  // get headman info for updates
  const getHeadmanInfo = async ()=> {
      try {
        const response = await axios.get(
          `${ApiUrl}/headman/single/${id}`,
          {
            headers: {
        ...
        if(response.data.success){
          const headman = response.data.data;
          console.log(headman)
          setValue('name',headman.name);
          setValue('contact',headman.contact);
          setValue('address',headman.address);
          setValue("gender", headman.gender);
          setValue("payment_status", headman.payment_status);
          setImage(headman.image?headman.image:'');
          setValue('notes',headman.notes)
        }
      } catch (error) {
        console.log(error);
  ...
  return (
    <div className="p-5 p-md-3 p-sm-2 mx-auto" style={{ maxWidth: "1200px" }}>
      <div className="d-flex justify-content-between align-items-center">
        <span className="h4">Headman Update</span>
        <button
