...
const { ApiUrl } = useContext(StoreContext);
  const getPatients = async (keywords = "", page = 1) => {
    setLoading(true);
    try {
      const response = await axios.get(
        `${
          ApiUrl
        }/patients/all/${keywords}?page=${page}&limit=${limit}`,
        {
          headers: {
    ...
    <div className="p-5 p-md-3 p-sm-2 mx-auto" style={{ maxWidth: "1000px" }}>
      ...
      >
        <input
          ...
          onKeyUp={(e) => getPatients(e.target.value)}
        />
      ...
      <table className="mt-5 border table table-striped">
        <thead className="border" style={{ backgroundColor: "#F5BCBA" }}>
          ...
          <th className="p-3">Headman</th>
          <th className="p-3">total_charges</th>
        ...
        <tbody>
          {loading ? (
            ...
            data.map((item, index) => (
              <ListPatients key={index} data={item} getPatients={getPatients} />
            ))
          )}
        </tbody>
      </table>
